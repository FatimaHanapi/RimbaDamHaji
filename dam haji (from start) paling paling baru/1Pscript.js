...
function drawBoard() {
    ...
    for(let j = 0; j < width/sideLengthOfSquare; j++) {
      ctx.fillStyle = ((i + j) % 2 == 0 ? "#8fbc8f" : "#556b2f");
...
function actHandler(x, y) {
  ...
  if(!killStreakOn && (board[y][x] != null && board[y][x].colorBoolean == redsTurn)) {
    if(x == lastClickedX && y == lastClickedY) {
      ...
      lastClickedX = -1;
      lastClickedY = -1;
      setResetHighLight(x, y, "#556b2f", false, redsTurn ? 1 : -1, true, board);
      ...
      if(lastClickedY != -1) {
        ...
        setResetHighLight(lastClickedX, lastClickedY, "#556b2f", false, redsTurn ? 1 : -1, true, board);
        if(potentialKillCoords.length != 0) potentialKillCoords = resetPotentialKillMarkerAndCasualty(potentialKillCoords);
      }
      selectedChecker(x, y);
...
function select(x, y) {
  ...
  setResetHighLight(x, y, "#556b2f", true, -1, false, simulatedArr);
...
function simulateMove(x, y) {
  // setResetHighLight(lastClickedX, lastClickedY, "#556b2f", false, redsTurn ? 1 : -1, true, board); Is reduntant?
